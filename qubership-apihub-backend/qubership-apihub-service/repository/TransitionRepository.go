...
func copyVersions(tx *pg.Tx, fromPkg, toPkg string) (int, error) {
	...
	copyVerDocs := "insert into published_version_revision_content (package_id, version, revision, checksum, index, file_id, path, slug, data_type, name, metadata, title, format, operation_ids, filename, shareability_status, api_kind) " +
		"(select ?, version, revision, checksum, index, file_id, path, slug, data_type, name, metadata, title, format, operation_ids, filename, shareability_status, api_kind from published_version_revision_content orig where orig.package_id = ?) on conflict do nothing"
